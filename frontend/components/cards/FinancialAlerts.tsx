  ...
  return (
    <div className="mb-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {/* Contas vencidas a pagar - VISUAL CRÍTICO */}
        {data.overdue_payables.count > 0 && (
          <div
            className="bg-red-50 border-2 border-red-500 rounded-lg p-4 cursor-pointer hover:bg-red-100 transition-colors shadow-md"
            onClick={() => onAlertClick?.('overdue_payables')}
          >
            <div className="flex items-center space-x-3 mb-2">
              <AlertCircle className="w-6 h-6 text-red-600" />
              <span className="text-sm font-bold text-red-800">⚠️ Contas Vencidas a Pagar</span>
            </div>
            <p className="text-lg font-bold text-red-700">{data.overdue_payables.count} conta(s)</p>
            <p className="text-sm font-semibold text-red-800">{formatCurrency(data.overdue_payables.value)}</p>
          </div>
        )}

        {/* Contas vencidas a receber - VISUAL CRÍTICO */}
        {data.overdue_receivables.count > 0 && (
          <div
            className="bg-red-50 border-2 border-red-500 rounded-lg p-4 cursor-pointer hover:bg-red-100 transition-colors shadow-md"
          ...
          >
            <div className="flex items-center space-x-3 mb-2">
              <AlertCircle className="w-6 h-6 text-red-600" />
              <span className="text-sm font-bold text-red-800">⚠️ Contas Vencidas a Receber</span>
            </div>
            <p className="text-lg font-bold text-red-700">{data.overdue_receivables.count} conta(s)</p>
            <p className="text-sm font-semibold text-red-800">{formatCurrency(data.overdue_receivables.value)}</p>
          </div>
        )}
