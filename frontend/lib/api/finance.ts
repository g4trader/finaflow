...
export const fetchForecastVsRealized = async (months: number = 6, year?: number): Promise<ForecastVsRealized> => {
  try {
    const params = new URLSearchParams({ months: String(months) });
    if (year) {
      params.set('year', String(year));
    }
    const data = await fetchWithAuth(`/api/v1/dashboard/operational/forecast-vs-realized?${params.toString()}`);
    return data;
  } catch (error) {
